...
cat ${DOC}.md | sed 's/^---$/\&nbsp;\n\n---\n\n\&nbsp;/' > $doc
pandoc --pdf-engine lualatex -H make-code-footnotesize.tex --include-in-header=nohyphen.cfg --highlight-style=highlight.theme -V geometry:margin=${MARGIN} -M "title=$TITLE" -M "author=$AUTHOR" -M "date=$DT" ${doc} style_${LANGUAGE}.yml -o ${DOC}_${DFN}.pdf
rm -f $doc
